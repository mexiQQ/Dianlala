...
- (void)viewDidLoad {
  ...
  [scanButton
      setFrame:CGRectMake((self.view.frame.size.width - 120) / 2,
                          (500 / (float)568) * self.view.frame.size.height, 120,
                          40)];
  ...
  [_Indicator
      setFrame:CGRectMake((self.view.frame.size.width - 30) / 2,
                          (self.view.frame.size.height - 30) / 2, 30, 30)];
  [_Indicator hidesWhenStopped];
  [self.view addSubview:_Indicator];

  UIImageView *imageView = [[UIImageView alloc]
      initWithFrame:CGRectMake((self.view.frame.size.width - 300) / 2, 100, 300,
                               300)];
  imageView.image = [UIImage imageNamed:@"pick_bg"];
  [self.view addSubview:imageView];

  _line = [[UIImageView alloc]
      initWithFrame:CGRectMake((self.view.frame.size.width - 220) / 2,
                               (110 / (float)568) * self.view.frame.size.height,
                               220, 2)];
...
- (void)animation1 {
  if (upOrdown == NO) {
    num++;
    _line.frame = CGRectMake(
        (self.view.frame.size.width - 220) / 2,
        (110 / (float)568) * self.view.frame.size.height + 2 * num, 220, 2);
    if (2 * num == 280) {
      upOrdown = YES;
    }
  } else {
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput
  ...
  NSDictionary *parameters = @{
    @"sname" : [[userDeault objectForKey:@"username"]
        stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],
    @"sid" : [userDeault objectForKey:@"password"],
    @"os" : @"iOS",
    @"pid" : [userDeault objectForKey:@"uuid"],
    @"sig" : [userDeault objectForKey:@"sig"]
  };
  [manager POST:stringValue
      ...
      success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (re.count == 3) {
          ...
          _Info = [NSString
              stringWithFormat:@"您已经在%@老师的%@课上点名成功",
                               [self UTF8_To_GB2312:tname],
                               [self UTF8_To_GB2312:cname]];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  if ([segue.identifier isEqualToString:@"signSuccess"]) {
    ValidSuccessViewController *destViewController =
        segue.destinationViewController;
    destViewController.Info = _Info;
  } else if ([segue.identifier isEqualToString:@"signFaile"]) {
    ValidFaileViewController *destViewController =
        segue.destinationViewController;
    destViewController.Info = _Info;
  } else {
  }
